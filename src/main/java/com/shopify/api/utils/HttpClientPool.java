...
import com.shopify.api.ShopifyConfig;

/**
 * @author Rahul Tiwari, Created on Aug 29, 2019 - 3:22:10 PM
 *
 */
public class HttpClientPool {
	...
	private HttpClientPool() {
		...
		cm.setMaxTotal(ShopifyConfig.getMaxHttpClientConnections());
		// Increase default max connection per route to 20
		cm.setDefaultMaxPerRoute(ShopifyConfig.getDefaultHttpClientMaxPerRoute());
		...
		RequestConfig requestConfig = RequestConfig.custom()
				.setConnectionRequestTimeout(ShopifyConfig.getHttpClientTimeoutMillis())
				.setSocketTimeout(ShopifyConfig.getHttpClientTimeoutMillis())
				.setConnectionRequestTimeout(ShopifyConfig.getHttpClientTimeoutMillis()).build();

		// Build the client.
		threadSafeClient = HttpClients.custom().setConnectionManager(cm).useSystemProperties().disableConnectionState()
